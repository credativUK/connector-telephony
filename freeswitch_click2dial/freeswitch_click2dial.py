...
#        if re.search('.*<(.*)>.*', values['callerid']) == None:
#            raise osv.except_osv(_('Format excpetion'), _('Caller ID needs to be formatted like NAME <NUMBER> (including the <>)'))
#        else:
#          return super(res_users, self).create(cr, uid, values, context=context)

#    def update(self, cr, uid, values, context=None):
#        if re.search('.*<(.*)>.*', values['callerid']) == None:
#            raise osv.except_osv(_('Format excpetion'), _('Caller ID needs to be formatted like NAME <NUMBER> (including the <>)'))
#        else:
#          return super(res_users, self).update(cr, uid, values, context=context)
        ...
        for user in self.browse(cr, uid, ids):
            ...
            for check_string in strings_to_check:
                if check_string[1]:
                    ...
                    if check_string[0] == 'Caller ID':
                        if re.search('.*<(.*)>.*', check_string[1]) == None:
                            raise osv.except_osv(_('Format excpetion'), _('Caller ID needs to be formatted like NAME <NUMBER> (including the <>)'))
...
class phone_common(orm.AbstractModel):
    ...
    def click2dial(self, cr, uid, erp_number, context=None):
	    ...
	    #if caller_number:
	    #  caller_number = caller_number.group(1).strip()
            if caller_name:
                if len(variable):
                    variable += ','
