...
class PhoneCommon(models.AbstractModel):
    ...
    def log_call_and_recording(self, cr, uid, odoo_type, odoo_src, odoo_dst, odoo_duration, odoo_start, odoo_filename, odoo_uniqueid, context=None):
        ...
        users = users_obj.search(cr, uid, [('sip_login', '=', caller_user)])
        if users:
        ...
        if odoo_filename:
            ...
            base_url = params.get_param(cr, uid, 'crm.voip.ucp_url', default='http://localhost/ucp?quietmode=1&module=cdr&command=download&msgid={odoo_uniqueid}&type=download&format=wav&ext={caller_user}')
            ir_attachment_data = {
